    ...
    <div className="container-page py-16 grid md:grid-cols-3 gap-10">
      <form onSubmit={submit} className="md:col-span-2 space-y-10">
        <div>
          ...
          <h1 className="font-serif text-3xl mb-6">주문자 정보</h1>
          <div className="grid grid-cols-2 gap-x-6 gap-y-5">
            ...
            <Field label="이메일" type="email" value={form.customer_email} onChange={(v) => setForm({ ...form, customer_email: v })} placeholder="주문 확인용 (선택)" className="col-span-2" />
          </div>
        </div>

        <div>
          <div className="eyebrow mb-3">— Step 2</div>
          <h2 className="font-serif text-3xl mb-6">배송지</h2>
          <div className="grid grid-cols-2 gap-x-6 gap-y-5">
            <Field label="우편번호" required value={form.shipping_postcode} onChange={(v) => setForm({ ...form, shipping_postcode: v })} placeholder="5자리" />
            <div />
            <Field label="주소" required value={form.shipping_address1} onChange={(v) => setForm({ ...form, shipping_address1: v })} className="col-span-2" />
            <Field label="상세주소" value={form.shipping_address2} onChange={(v) => setForm({ ...form, shipping_address2: v })} className="col-span-2" />
            <Field label="배송 메모" value={form.shipping_memo} onChange={(v) => setForm({ ...form, shipping_memo: v })} className="col-span-2" />
          </div>
          {remote && (
        ...
        <div>
          ...
          <h2 className="font-serif text-3xl mb-6">결제 (무통장입금)</h2>
          <div className="grid grid-cols-2 gap-x-6 gap-y-5">
