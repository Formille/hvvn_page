      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-14 max-w-4xl mx-auto">
        {products.map((p, i) => (
          <Link key={p.id} href={`/products/${p.slug}`} className="group flex flex-col items-center text-center">
            ...
            <div className="mt-5 chrome-text text-2xl md:text-3xl leading-none">{p.name}</div>
            <div className="mt-1 text-sm chrome-text-soft tracking-wider">{formatKRW(p.price_krw)}</div>
          </Link>
        ))}
        {products.length === 0 && (
